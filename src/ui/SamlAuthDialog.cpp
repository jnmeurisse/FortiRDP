...
namespace ui {
	...
	INT_PTR SamlAuthDialog::onCreateDialogMessage(WPARAM wParam, LPARAM lParam)
	{
		...
		auto web_core_created = [this](HRESULT result, ICoreWebView2Controller* controller) {
			if (FAILED(result)) {
				...
				try {
					...
					// Navigate to URL
					std::string status_message = aux::string_format("Connecting to %s://%s",
						_saml_auth_info.service_provider_url.get_scheme().c_str(),
						_saml_auth_info.service_provider_url.get_authority().c_str()
					);

					set_control_text(IDC_SAML_STATUS, aux::str2wstr(status_message));
					hr = web_view->Navigate(aux::str2wstr(_saml_auth_info.service_provider_url.to_string(false)).c_str());
					if (FAILED(hr))
		...
		std::wstring app_data_path = aux::Path::get_appdata_path().folder() + L"FortiRDP\\";
		...
		if (!sender || !args) {
			...
			try {
				...
				if (!is_success) {
					...
					const http::Url source_url{ aux::wstr2str(source_uri.get()) };
					if (aux::iequal(source_url.get_hostname(), _saml_auth_info.service_provider_url.get_hostname())) {
						hr = _web_cookie_manager->GetCookies(
							aux::str2wstr(_saml_auth_info.service_provider_url.to_string(false)).c_str(),
	...
	)
	{
		...
		if (!sender || !args) {
			...
			try {
				...
				http::Url web_server_url{ aux::wstr2str(get_control_text(IDC_SAML_STATUS)) };
				if (aux::iequal(web_server_url.get_authority(), _saml_auth_info.service_provider_url.get_authority())) {
					...
					if (aux::iequal(aux::wstr2str(certificate_pem.get()), _saml_auth_info.service_provider_crt)) {
						hr = args->put_Action(
	...
	static http::Cookie WebviewCookie2Cookie(ICoreWebView2Cookie& webview2_cookie)
	{
		...
		return http::Cookie(
			aux::wstr2str(cookie_name.get()),
			aux::obfstring(aux::wstr2str(cookie_value.get())),
			aux::wstr2str(cookie_domain.get()),
			aux::wstr2str(cookie_path.get()),

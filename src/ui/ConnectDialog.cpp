...
namespace ui {
	...
	void ConnectDialog::connect(bool clear_log)
	{
		...
		_task_info = std::make_unique<utl::TaskInfo>(task_name, task_params);

		// Check if the app executable exists.
		if (!task_name.empty() && !utl::file_exists(task_name)) {
		...
		if (!_params.rdp_filename().empty() && !utl::file_exists(_params.rdp_filename())) {
		...
		if (_params.us_cert_filename().length() > 0) {
			...
			user_crt = utl::Path(_params.us_cert_filename());
		...
		case TIMER_COUNTERS:
			if (tunneler) {
				...
				const std::string message = utl::str::string_format(
					...
					sent_rcvd_counters.rcvd / 1024.0,
					client_info.c_str()
				);
	...
	INT_PTR ConnectDialog::onAsyncMessage(UINT eventId, void* param)
	{
		...
		if (eventId == AsyncMessage::ShowCredentialsDialogRequest->id()) {
			...
			onOutputInfoEvent(static_cast<utl::LogQueue*>(param));

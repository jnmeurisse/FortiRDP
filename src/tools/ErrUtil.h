...
namespace tools {
	...
	// mbed tls error codes defined in mbedccl/err.h
	//using mbed_err = ::mbed_err;
	...
	public:
		explicit mbed_error(mbed_errnum errnum) : _errnum(errnum >= 0 ? 0 : errnum) {};
		
		std::string message() const noexcept override;

	private:
		const mbed_errnum _errnum;
	};

...
{
	netctx_poll_status status = { 0, MBEDTLS_ERR_SSL_BAD_INPUT_DATA };
	mbedtls_net_context* const net_context = (mbedtls_net_context*)ctx;
	...
	if (net_context) {
		...
		int rc = mbedtls_net_poll(net_context, rw, timeout);
		if (rc < 0) {
			status.errnum = rc;
		}
		else if (rc == 0) {
			...
			status.errnum = 0;
			status.poll_event.read = rc & MBEDTLS_NET_POLL_READ;
			status.poll_event.write = rc & MBEDTLS_NET_POLL_WRITE;
		}
	}

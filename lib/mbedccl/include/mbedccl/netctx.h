 ...
 *    status_code : .
 *         NETCTX_POLL_OK : the operation succeeded. The event flags
 *                          indicates if the context is ready for
 *                          read or write.
 *         NETCTX_POLL_TIMEOUT : indicates that the call timed out
 *                               before the context became ready for
 *                               read or write.
 *         NETCTX_POLL_ERROR : An error has occurred.  The errnum
 *                             field contains the rror code.
 ...
 *     sbytes      | > 0    | = 0       | = 0
 *
 *  SND_OK : The send operation succeeded, sbytes contains the number of bytes
 *           effectively sent.
 ...
 *     rbytes      | > 0    | = 0       | = 0       | = 0
 *
 *  RCV_OK : The receive operation succeeded, rbytes contains the number of bytes
 *           received.
 *  RCV_RETRY : No bytes have been received, the receive operation must be retried.
 *              The errnum field contains the mbed TLS error code.
 *  RCV_ERROR : No bytes have been received, the receive operation failed.
 *              The errnum field contains the mbed TLS error code.
 *  RCV_EOF   : No bytes have been received, the peer closed the network context.
 *
 */
